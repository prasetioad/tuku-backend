  ...
  return new Promise((resolve, reject) => {
    connection.query(
      ...
      (err, resultCount) => {
        ...
        connection.query(
          `SELECT bag.id, product.image, product.title, store.name AS brand, bag.size, bag.qty, bag.price FROM ((bag INNER JOIN product ON bag.idProduct = product.id) INNER JOIN store ON bag.idStore = store.id) WHERE bag.idUser = ? ORDER BY ${sortBy} ${orderBy} LIMIT ?, ?`,
