...
exports.findStore = (userID) => {
    return new Promise((resolve, reject) => {
        connection.query("SELECT * FROM store WHERE idUser = ?", userID,
            (err, result) => {
                if (!err) {
                    resolve(result)
                } else {
    ...
    return new Promise((resolve, reject) => {
        connection.query(
            ...
            (err, result) => {
                ...
                connection.query(`SELECT * FROM product WHERE idStore = ? ORDER BY ${sortBy} ${order} LIMIT ?,?`,
...
exports.findProduct = (idStore, title) => {
    return new Promise((resolve, reject) => {
        connection.query(`SELECT COUNT(*) AS totalProduct FROM product WHERE idStore = ? AND title = ?`, [idStore, title],
            (err, result) => {
                if (!err) {
                    resolve(result)
                } else {
